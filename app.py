...
# [FIX] Menggunakan seluruh 23 fitur (20 asli + 3 engineered) agar
#       dimensi input sesuai dengan model yang dilatih.
...
# ──────────────────────────────────────────────
# Nilai min/max diambil dari rentang dataset (0–5 untuk fitur ordinal)
# sehingga normalisasi konsisten saat inferensi satu sampel.
MINMAX_RANGE = {
    "study_load":              (0, 5),
...
def build_input_row(raw: dict) -> pd.DataFrame:
    ...
    """
    # ── Korelasi Pearson aktual (dihitung saat training) ──────────────
    # Nilai ini statis agar tidak perlu memuat ulang dataset saat runtime
    w_study  = 0.6342
    w_career = 0.7426
    w_acad   = 0.7209
    ...
    # 1. Academic Stress Index (correlation-weighted)
    ...
    # Urutkan kolom sesuai selected_features (PENTING agar dimensi cocok)
